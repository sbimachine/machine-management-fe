...
export default function TeknisiRiwayatTable() {
	const { penugasan: penugasanState, teknisi, setPenugasanTable, setPenugasan } = useStore();
	const { table } = penugasanState;

	const [kategori, penugasan] = useQueries({
		queries: [
			{ queryKey: ['kategori'], queryFn: browseKategori },
			{ queryKey: ['penugasan', { ...table.filter, userId: teknisi.selectedData?.id }], queryFn: browsePenugasan },
		],
	});
